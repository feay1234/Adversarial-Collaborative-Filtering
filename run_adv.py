...
def training(model, dataset, args, runName, epoch_start, epoch_end, time_stamp):  # saver is an object to save pq
    with tf.Session() as sess:
        ...
        if args.adver:
            ckpt_save_path = args.path + "Pretrain/%s/APR/embed_%d/%s/" % (args.dataset, args.embed_size, time_stamp)
            ckpt_restore_path = args.path + "Pretrain/%s/MF_BPR/embed_%d/%s/" % (
                args.dataset, args.embed_size, time_stamp)
        else:
            ckpt_save_path = args.path + "Pretrain/%s/MF_BPR/embed_%d/%s/" % (args.dataset, args.embed_size, time_stamp)
            ckpt_restore_path = 0 if args.restore is None else args.path + "Pretrain/%s/MF_BPR/embed_%d/%s/" % (
