...
class GRU4Rec2:
    ...
    def train(self, x_train, y_train, batch_size):
        ...
        for u in self.trainSeq:
            if len(self.trainSeq[u]) < 2:
                continue
            for idx in range(len(self.trainSeq[u][:-1])):
                seq.append(self.trainSeq[u][idx])
                label.append(self.trainSeq[u][idx + 1])

                if len(seq) == batch_size:
                    feed_dict = {self.X: seq, self.Y: label}
                    loss, _ = self.sess.run([self.cost, self.train_op], feed_dict)
                    losses.append(loss)
                    seq = []
                    label = []
